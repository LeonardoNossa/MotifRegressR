...
#' @export
plot_dimensionality_reduction <- function(compendium, metadata = NULL, 
                                          color_by = NULL, method = "PCA"){
  ...
  if (method == "PCA") {
    ...
    axis_labels <- c(paste0("PC1 (", explained_variance[1], "%)"), 
                     paste0("PC2 (", explained_variance[2], "%)"))
  } else if (method == "MDS") {
    ...
    tsne <- Rtsne::Rtsne(data_transposed, dims = 2, perplexity = 30, 
    ...
    umap_result <- umap::umap(data_transposed, n_neighbors = 15, 
    ...
    stop("Method not supported. Please choose among 'PCA', 'MDS', 't-SNE', 
         or 'UMAP'.")
  }
  
  reduction_result <- 
    reduction_result %>% dplyr::mutate(Sample = rownames(data_transposed))
  
  if (is.null(metadata) && is.null(color_by)) {
    reduction_result <- 
      reduction_result %>% dplyr::mutate(Tooltip = paste0("run_accession: ",
                                                          colnames(compendium)))
    ...
    el.on('plotly_selected', function(data) {
      if (data && data.points) {
        var selected_accessions = data.points.map(pt => {
          var tooltip = pt.text || '';
          var accession_match = tooltip.match(/run_accession: 
          ([^<]+?)(?=<br>|$)/);
...
");
  ...
  if (is.null(color_by)) {
    tooltip_text <- apply(metadata, 1, function(row) {
      to_keep <- c("study_accession", "tax_id", "scientific_name", 
                   "instrument_model", "sample_title")
      paste(
    ...
    reduction_data <- reduction_result %>% dplyr::left_join(metadata, 
                                                            by = "Sample")
  ...
  palette_colors <- 
    grDevices::colorRampPalette(RColorBrewer::brewer.pal(min(8, num_groups), 
                                                         "Set1"))(num_groups)
