...
The function uses regions corresponding to genesdefined in a GFF file 
...
- The rownames of the matrix correspond to the gene IDs
...
production of more reliable results, since Transcription Factor binding on the DNA
should be a matter of 3D interactions.
...
- The rownames will be the gene IDs
...
The shape of the compendium dataframe should be N x K, where N is the number of genes
...
```
The function returns a dataframe of size (at most) K x 27, where K is the number of samples
If metadata for some samples cannot be retrieved the metadata dataframe will have less than K rows
...
(and most similar conditions aswell), the user can use this function to build a clustermap.
