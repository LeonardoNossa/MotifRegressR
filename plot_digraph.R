...
#' @importFrom magrittr %>%
#' @export
plot_digraph <- function(motif_names, edges_sublist){

  `%>%` <- magrittr::`%>%`

  regression_name <- names(edges_sublist)
  edges <- edges_sublist[[1]]
  condition_names <- unique(edges$conditions)

  nodes <- data.frame(
    name = c(motif_names, condition_names),
    type = c(rep(FALSE, length(motif_names)), rep(TRUE,
  ...
  node_positions <- data.frame(
    name = c(motif_names, condition_names),
    x = c(seq(1, 5, length.out = length(motif_names)),
          seq(1, 5, length.out = length(condition_names))),
  ...
  edge_positions$Info <- paste0(
    "<br>", edges$motifs, " ⇔ ", edges$conditions, "<br>Importance: ",
    edges$coef
  )

  node_positions$Info <- paste0(
    "<br>","Node: ", node_positions$name, "<br>Type: ",
    ifelse(node_positions$type, "Condition", "Motif")
  )
  ...
  gg <- ggplot2::ggplot() +
    ...
    ggplot2::scale_fill_manual(values = c("#3CB371", "aquamarine2")) +
    ggplot2::scale_color_gradient(low = "#F1D0FF", high = "#4B0082") +
  ...
  plotly_graph <- plotly::ggplotly(gg, tooltip = c("Info")) %>%
    plotly::layout(
      title = list(
        text = paste0(
          "K most important motifs bipartite graph",
          "<span style='font-size:16px;'>", paste0(" (",regression_name,")"), "</span>"
        ),
        x = 0.5,
        xanchor = "center",
        y = 0.99,
